...
export class Installer {
    ...
    public setup(platform: NodeJS.Platform): Promise<string | void> {
    ...
    public deleteInstallationArchive() {
        return new Promise((resolve, reject) => {
            const platform = this[this.platform]

            if (typeof platform !== 'string')
                fs.unlink(platform.archive, error => {
    ...
    private installLinux(): Promise<string | void> {
        ...
        return this.exec(`${HandbrakeCLIPath} --version`).then(result => {
            if (this.checkVersion(result)) return HandbrakeCLIPath
                ...
                exec(`hdiutil attach ${archive}`, (err, stdout) => {
                    ...
                    if (match) {
                        ...
                        let source = fs.createReadStream(copyFrom),
                            destination = fs.createWriteStream(copyTo, {
                                mode: parseInt('755', 8)
                            })
...
if (options['install']) {
    new Installer().setup(process.platform).then(() => {
